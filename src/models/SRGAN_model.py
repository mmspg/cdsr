...
from compressai.zoo import mbt2018_mean, bmshj2018_factorized, bmshj2018_hyperprior
...
def pad_m64(img):
    ...
    if img.shape[2]%64 != 0:
        new_x = 64 * int(img.shape[2]/64+1)
    else:
        new_x = img.shape[2]

    if img.shape[3]%64 != 0:
        new_y = 64 * int(img.shape[3]/64+1)
    else:
        new_y = img.shape[3]

    img_holder = torch.zeros((img.shape[0],img.shape[1],new_x,new_y))
    img_holder[:,:,:img.shape[2], :img.shape[3]] = img
    return img_holder
...
class SRGANModel(BaseModel):
    def __init__(self, opt):
        ...
        self.compressor = bmshj2018_hyperprior(quality=opt['compression_quality'] + 5, pretrained=True).eval()
    ...
    def feed_data(self, data, need_GT=True, is_val = False):
        ...
        data['LQ'] = self.compressor.forward(pad_m64(data['LQ']))['x_hat']
    ...
    # Takes HR image, downsamples to LR and compresses
    def compress(self, image):
        dat = imresize(image, 1.0 / 4, True)
        dat = self.compressor.compress(pad_m64(dat))
        return dat['strings'], dat['shape']


    # Takes compressed form of LR and reconstructs HR'
    def decompress(self, strings, shape):
        lr = self.compressor.decompress(strings, shape)['x_hat']
        dat = {'LQ':lr}
        self.feed_data(dat, is_val=True)
        self.test()
        return util.tensor2img(self.fake_H.detach()[0].float().cpu())